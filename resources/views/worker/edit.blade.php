...
@section('content')
    ...
    <section class="lot_add_form px-3 old_form">
        <h2 class="text-center">Edit Worker</h2>
        
        <div class="flash_messages">
            @include("partials.flash_messages")
        </div>

        <form action={{ route("worker.edit", ["id" => $record->id]) }} method="POST" class="mt-3">
            @csrf()
                    ...
                    <div class="col-8">
                        <select class="w-100" name="worker_type_id" id="worker_type_id">
                            ...
                            @foreach ($workerTypes as $type)
                                <option value="{{ $type["id"] }}" {{ old("worker_type_id", @$record->worker_type_id) == $type["id"] ? "selected" : "" }}>{{ @$type["title"] }}</option>
                            @endforeach
                        </select>
                        <span class="text-danger">
            ...
            <div class="form_group mb-2">
                <div class="row">
                    ...
                    <div class="col-8">
                        <input class="w-100" type="text" name="phonenumber" id="phonenumber" value="{{ old("phonenumber", @$record->phonenumber) }}">
                        <span class="text-danger">
                            @error('phonenumber')
                                {{ $message }}
                            @enderror
